...
pub fn is_zero(vector: &[I64F64]) -> bool {
    vector.iter().sum::<I64F64>() == I64F64::from_num(0)
}

pub fn normalize(x: Vec<I64F64>) -> Vec<I64F64> {
    let sum: I64F64 = x.iter().sum();
    normalize_with_sum(x, sum)
}

#[inline]
pub fn normalize_with_sum(mut x: Vec<I64F64>, sum: I64F64) -> Vec<I64F64> {
...
pub fn u16_proportion_to_fixed(x: u16) -> I64F64 {
    I64F64::from_num(x).saturating_div(I64F64::from_num(u16::MAX))
}

pub fn fixed_proportion_to_u16(x: I64F64) -> u16 {
    (x.saturating_mul(I64F64::from_num(u16::MAX))).to_num()
}

// Return a new sparse matrix with a masked out diagonal of input sparse matrix.
pub fn mask_diag_sparse(sparse_matrix: &[Vec<(u16, I64F64)>]) -> Vec<Vec<(u16, I64F64)>> {
    let n: usize = sparse_matrix.len();
    let mut result: Vec<Vec<(u16, I64F64)>> = vec![vec![]; n];
...
pub fn mat_ema_alpha_vec_sparse(
    ...
    let zero = I64F64::from_num(0.0);
    let one = I64F64::from_num(1.0);

    new.iter()
        .zip(old)
        .map(|(new_row, old_row)| {
            let mut row_map: BTreeMap<u16, I64F64> = new_row
                .iter()
                .map(|&(j, value)| {
...
pub fn calculate_logistic_params(
    alpha_high: I64F64,
    alpha_low: I64F64,
    consensus_high: I64F64,
    consensus_low: I64F64,
) -> (I64F64, I64F64) {
    if consensus_high <= consensus_low
        || alpha_low == I64F64::from_num(0)
        || alpha_high == I64F64::from_num(0)
    {
        return (I64F64::from_num(0), I64F64::from_num(0));
    }

    let calc_term = |alpha: I64F64| {
        ln((I64F64::from_num(1).saturating_div(alpha)).saturating_sub(I64F64::from_num(1)))
            .unwrap_or(I64F64::from_num(0.0))
    };
...
pub fn compute_alpha_values(consensus: &[I64F64], a: I64F64, b: I64F64) -> Vec<I64F64> {
    let alpha: Vec<I64F64> = consensus
        .iter()
        .map(|&c| {
            let exp_val =
                exp(b.saturating_sub(a.saturating_mul(c))).unwrap_or(I64F64::from_num(0.0));
            I64F64::from_num(1.0).saturating_div(I64F64::from_num(1.0).saturating_add(exp_val))
        })
        .collect();
...
pub fn quantile(data: &[I64F64], quantile: f64) -> I64F64 {
    ...
    if low == high {
        sorted.get(low).cloned().unwrap_or(I64F64::from_num(0))
    } else {
        sorted
            .get(low)
            .cloned()
            ...
            .lerp(
                sorted.get(high).cloned().unwrap_or(I64F64::from_num(0)),
                I64F64::from_num(pos.fract()),
            )
    }
}

/// Normalizes (sum to 1 except 0) each row (dim=0) of a sparse matrix in-place.
pub fn inplace_row_normalize_sparse(sparse_matrix: &mut [Vec<(u16, I64F64)>]) {
    for sparse_row in sparse_matrix.iter_mut() {
        let row_sum: I64F64 = sparse_row.iter().map(|(_j, value)| *value).sum();
        if row_sum != I64F64::from_num(0) {
            sparse_row
                .iter_mut()
...
pub fn matmul_sparse(
    sparse_matrix: &[Vec<(usize, I64F64)>],
    vector: &[I64F64],
    columns: usize,
) -> Vec<I64F64> {
    let mut result: Vec<I64F64> = vec![I64F64::from_num(0.0); columns];
    for (i, sparse_row) in sparse_matrix.iter().enumerate() {
        for (j, value) in sparse_row.iter() {
...
pub fn linear_consensus(
    stake: &[I64F64],
    score: &[Vec<(u16, I64F64)>],
    columns: u16,
) -> Vec<I64F64> {
    let zero: I64F64 = I64F64::from_num(0);
    let mut consensus: Vec<I64F64> = vec![zero; columns as usize];
    let mut total_stake: I64F64 = zero;
...
pub fn weighted_median_col_sparse(
    ...
    let stake_sum: I64F64 = use_stake.iter().sum();
    let stake_idx: Vec<usize> = (0..use_stake.len()).collect();
    let minority: I64F64 = stake_sum.saturating_sub(majority);
    let mut use_score: Vec<Vec<I64F64>> = vec![vec![zero; use_stake.len()]; columns as usize];

    let mut median: Vec<I64F64> = vec![zero; columns as usize];
    let mut k: usize = 0;
...
pub fn weighted_median(
    ...
    let mut lo_stake: I64F64 = I64F64::from_num(0);
    let mut hi_stake: I64F64 = I64F64::from_num(0);
...
pub fn row_sum_sparse(sparse_matrix: &[Vec<(u16, I64F64)>]) -> Vec<I64F64> {
    let rows = sparse_matrix.len();
    let mut result: Vec<I64F64> = vec![I64F64::from_num(0); rows];
    for (i, sparse_row) in sparse_matrix.iter().enumerate() {
...
pub fn col_clip_sparse(
    sparse_matrix: &[Vec<(u16, I64F64)>],
    col_threshold: &[I64F64],
) -> Vec<Vec<(u16, I64F64)>> {
    let mut result: Vec<Vec<(u16, I64F64)>> = vec![vec![]; sparse_matrix.len()];
    for (i, sparse_row) in sparse_matrix.iter().enumerate() {
        for (j, value) in sparse_row.iter() {
...
pub fn inplace_mask_vector(mask: &[bool], vector: &mut [I64F64]) {
    if mask.is_empty() {
        return;
    }

    assert_eq!(mask.len(), vector.len());
    let zero: I64F64 = I64F64::from_num(0.0);
...
pub fn vec_fixed64_to_fixed32(vec: Vec<I64F64>) -> Vec<I64F64> {
    vec.into_iter().map(I64F64::from_num).collect()
}

pub fn is_topk(vector: &[I64F64], k: usize) -> Vec<bool> {
    ...
    idxs.sort_by_key(|&idx| *vector.get(idx).unwrap_or(&I64F64::from_num(0))); // ascending stable sort
...
pub fn col_normalize_sparse(
    mut sparse_matrix: Vec<Vec<(usize, I64F64)>>,
    columns: usize,
) -> Vec<Vec<(usize, I64F64)>> {
    let mut col_sum: Vec<I64F64> = vec![I64F64::from_num(0); columns]; // assume square matrix, rows=cols
    ...
    for sparse_row in &mut sparse_matrix {
        for (j, value) in sparse_row {
            ...
            if *col_sum_j == I64F64::from_num(0.) {
                continue;
            }
...
pub fn row_hadamard_sparse(
    ...
    let mut result: Vec<Vec<(usize, I64F64)>> = sparse_matrix.to_vec();
...
pub fn matmul_transpose_sparse(
    ...
    let mut result: Vec<I64F64> = vec![I64F64::from_num(0); sparse_matrix.len()];
    for (i, sparse_row) in sparse_matrix.iter().enumerate() {
        for (j, value) in sparse_row {
...
pub fn mat_ema_sparse(
    ...
    let zero: I64F64 = I64F64::from_num(0.0);
    let one_minus_alpha: I64F64 = I64F64::from_num(1.0).saturating_sub(alpha);
    let mut result: Vec<Vec<(u16, I64F64)>> = vec![vec![]; n];
    for i in 0..new.len() {
        let mut row: Vec<I64F64> = vec![zero; n];
...
/// input.
pub fn vec_max_upscale_to_u16(vec: &[I64F64]) -> Vec<u16> {
    let u16_max: I64F64 = I64F64::from_num(u16::MAX);
    let threshold: I64F64 = I64F64::from_num(32768);
    let max_value: Option<&I64F64> = vec.iter().max();
    match max_value {
        Some(val) => {
            if *val == I64F64::from_num(0) {
                return vec
                    .iter()
                    .map(|e: &I64F64| e.saturating_mul(u16_max).to_num::<u16>())
                    .collect();
            }
            if *val > threshold {
                return vec
...
pub fn vecdiv(x: &[I64F64], y: &[I64F64]) -> Vec<I64F64> {
...
pub fn inplace_col_max_upscale_sparse(sparse_matrix: &mut [Vec<(u16, I64F64)>], columns: u16) {
    let mut col_max: Vec<I64F64> = vec![I64F64::from_num(0.0); columns as usize]; // assume square matrix, rows=cols
    ...
    for sparse_row in sparse_matrix.iter_mut() {
        for (j, value) in sparse_row.iter_mut() {
            ...
            if *col_max_j == I64F64::from_num(0.) {
                continue;
            }
...
mod tests {
    ...
    macro_rules! fixed_vec {
        ...
        ($($x:expr),+ $(,)?) => {
            vec![$(I64F64::from_num($x)),+]
        };
    }

    /// Reshape vector to sparse matrix with specified number of input rows, cast f32 to I64F64.
    fn vec_to_sparse_mat_fixed(
        ...
        let mut mat: Vec<Vec<(u16, I64F64)>> = vec![];
        if transpose {
            for col in 0..cols {
                let mut row_vec: Vec<(u16, I64F64)> = vec![];
                for row in 0..rows {
                    if vector[row * cols + col] > 0. {
                        row_vec.push((row as u16, I64F64::from_num(vector[row * cols + col])));
                    }
                }
                mat.push(row_vec);
            }
            ...
            for row in 0..rows {
                ...
                for col in 0..cols {
                    if vector[row * cols + col] > 0. {
                        row_vec.push((col as u16, I64F64::from_num(vector[row * cols + col])));
                    }
                }
                mat.push(row_vec);
            }
    ...
    fn assert_float_compare(a: I64F64, b: I64F64, epsilon: I64F64) {
        assert!(I64F64::abs(a - b) <= epsilon, "a({a:?}) != b({b:?})");
    }

    fn assert_vec_compare(va: &[I64F64], vb: &[I64F64], epsilon: I64F64) {
    ...
    fn assert_sparse_mat_compare(
        ...
        for row in 0..ma.len() {
            ...
            for j in 0..ma[row].len() {
                ...
                assert_float_compare(ma[row][j].1, mb[row][j].1, epsilon) // I64F64
            }
        }
    }
    ...
    #[test]
    fn test_math_to_num() {
        let val: I64F64 = I64F64::from_num(u16::MAX);
        let res: u16 = val.to_num::<u16>();
        assert_eq!(res, u16::MAX);
        let vector: Vec<I64F64> = vec![val; 1000];
        let target: Vec<u16> = vec![u16::MAX; 1000];
        let output: Vec<u16> = vector.iter().map(|e: &I64F64| e.to_num::<u16>()).collect();
        assert_eq!(output, target);
        let output: Vec<u16> = vector
            .iter()
            .map(|e: &I64F64| (*e).to_num::<u16>())
            .collect();
        assert_eq!(output, target);
        let val: I64F64 = I64F64::max_value();
        let res: u64 = val.to_num::<u64>();
        let vector: Vec<I64F64> = vec![val; 1000];
        let target: Vec<u64> = vec![res; 1000];
        let output: Vec<u64> = vector.iter().map(|e: &I64F64| e.to_num::<u64>()).collect();
        assert_eq!(output, target);
        let output: Vec<u64> = vector
            .iter()
            .map(|e: &I64F64| (*e).to_num::<u64>())
            .collect();
        assert_eq!(output, target);
        let val: I64F64 = I64F64::from_num(0);
        let res: u64 = val.to_num::<u64>();
        let vector: Vec<I64F64> = vec![val; 1000];
        let target: Vec<u64> = vec![res; 1000];
        let output: Vec<u64> = vector.iter().map(|e: &I64F64| e.to_num::<u64>()).collect();
    ...
    fn test_math_vec_to_sparse_mat_fixed() {
        ...
        let target: Vec<Vec<(u16, I64F64)>> = vec![
            vec![(1, I64F64::from_num(1.)), (2, I64F64::from_num(2.))],
            vec![(1, I64F64::from_num(10.)), (2, I64F64::from_num(100.))],
        ];
        let mat = vec_to_sparse_mat_fixed(&vector, 2, false);
        assert_sparse_mat_compare(&mat, &target, I64F64::from_num(0));
        let vector: Vec<f32> = vec![0., 0.];
        let target: Vec<Vec<(u16, I64F64)>> = vec![vec![], vec![]];
        let mat = vec_to_sparse_mat_fixed(&vector, 2, false);
        assert_sparse_mat_compare(&mat, &target, I64F64::from_num(0));
        let vector: Vec<f32> = vec![0., 1., 2., 0., 10., 100.];
        let target: Vec<Vec<(u16, I64F64)>> = vec![
            vec![],
            vec![(0, I64F64::from_num(1.)), (1, I64F64::from_num(10.))],
            vec![(0, I64F64::from_num(2.)), (1, I64F64::from_num(100.))],
        ];
        let mat = vec_to_sparse_mat_fixed(&vector, 2, true);
        assert_sparse_mat_compare(&mat, &target, I64F64::from_num(0));
        let vector: Vec<f32> = vec![0., 0.];
        let target: Vec<Vec<(u16, I64F64)>> = vec![vec![]];
        let mat = vec_to_sparse_mat_fixed(&vector, 2, true);
        assert_sparse_mat_compare(&mat, &target, I64F64::from_num(0));
    }
    ...
    fn test_math_normalize() {
        ...
        let x: Vec<I64F64> = vec![];
        let y: Vec<I64F64> = normalize(x.clone());
        assert_vec_compare(&x, &y, epsilon);
        let x: Vec<I64F64> = fixed_vec![1.0, 10.0, 30.0,];
        let y: Vec<I64F64> = normalize(x.clone());
        assert_vec_compare(
            &y,
            &[
        ...
        assert_float_compare(y.iter().sum(), I64F64::from_num(1.0), epsilon);
        let x: Vec<I64F64> = fixed_vec![-1.0, 10.0, 30.0];
        let y: Vec<I64F64> = normalize(x.clone());
        assert_vec_compare(
            &y,
