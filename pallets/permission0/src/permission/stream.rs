...
pub struct StreamScope<T: Config> {
    ...
    /// Funnel operations enabled for this stream permission.
    ///
    /// A funnel consumes all incoming streams and outputs a single, unique stream ID.
    /// This new stream is the one forwarded to the recipients, allowing for local
    /// specialization of streams and separation of concerns.
    ///
    /// The current implementation expects this list to be limited to 1.
    pub funnels: BoundedVec<FunnelStream<T>, ConstU32<1>>,
}

impl<T: Config> StreamScope<T> {
    pub fn enable_funnel(&mut self, permission_id: PermissionId) -> DispatchResult {
        ensure!(self.funnels.is_empty(), Error::<T>::TooManyStreamFunnels);

        let derived_stream = pallet_permission0_api::generate_funnel_stream_id(&permission_id, 1);

        self.funnels
            .try_push(FunnelStream {
                derived_stream,
                nonce: 1,
                created_at: polkadot_sdk::frame_system::Pallet::<T>::block_number(),
            })
            .map_err(|_| Error::<T>::TooManyStreamFunnels)?;

        Pallet::<T>::deposit_event(Event::FunnelEnabled {
            permission_id,
            derived_stream_id: derived_stream,
        });

        Ok(())
    }

    pub fn disable_funnel(&mut self, permission_id: PermissionId) -> DispatchResult {
        if let Some(funnel) = self.funnels.first() {
            Pallet::<T>::deposit_event(Event::FunnelDisabled {
                permission_id,
                derived_stream_id: funnel.derived_stream,
            });
        }
...
pub struct FunnelStream<T: Config> {
    /// Output stream generated by this funnel.
    pub derived_stream: StreamId,
    /// Nonce used when generating the derived stream.
    pub nonce: u32,
    /// Block at which the funnel was created.
...
pub(crate) fn do_accumulate_streams<T: Config>(
    ...
    let streams: polkadot_sdk::sp_std::vec::Vec<_> =
        AccumulatedStreamAmounts::<T>::iter_prefix((agent, stream_id)).collect();
...
pub(crate) fn do_distribute_stream<T: Config>(
    ...
    match &stream_scope.allocation {
        StreamAllocation::Streams(streams) => {
            let streams: polkadot_sdk::sp_std::vec::Vec<_> = streams
                .keys()
            ...
            for (source_stream, mut imbalance) in streams {
                let target_stream = stream_scope
                    .funnels
                    .first()
                    .map_or(source_stream, |funnel| &funnel.derived_stream);

                do_distribute_to_targets::<T>(
                    &mut imbalance,
                    permission_id,
                    stream_scope.recipients.iter(),
                    Some(source_stream),
                    Some(target_stream),
                    total_weight,
                    reason,
                ...
                if !remainder.is_zero() {
                    AccumulatedStreamAmounts::<T>::mutate(
                        (&contract.delegator, source_stream, permission_id),
...
fn do_distribute_to_targets<'a, T: Config>(
    imbalance: &mut NegativeImbalanceOf<T>,
    permission_id: PermissionId,
    recipients: impl Iterator<Item = (&'a T::AccountId, &'a u16)>,
    _do_not_use_source_stream: Option<&StreamId>,
    ...
    for (target, weight) in recipients {
        ...
        Pallet::<T>::deposit_event(Event::StreamDistribution {
            permission_id,
            source_stream: _do_not_use_source_stream.cloned(),
            target_stream: stream.cloned(),
