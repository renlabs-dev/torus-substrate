use pallet_permission0::{Error, Permissions};
...
fn revoke_children() {
    new_test_ext().execute_with(|| {
        ...
        let permission_id_2 = assert_ok!(Permission0::grant_emission_permission(
            agent_0,
            agent_1,
            pallet_permission0_api::EmissionAllocation::FixedAmount(to_nano(10)),
            vec![(agent_1, u16::MAX / 2), (agent_2, u16::MAX / 2)],
        ...
        Permissions::<Test>::mutate(permission_id_2, |contract| {
            contract.as_mut().unwrap().parent = Some(permission_id);
        });
        ...
        assert!(!Permissions::<Test>::contains_key(permission_id_2));
    });
}
