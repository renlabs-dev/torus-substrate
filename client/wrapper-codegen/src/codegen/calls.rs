...
fn generate_pattern_wrappers(network: &InterfaceSource, pattern: &CallPattern) -> TokenStream {
    ...
    if !UNSIGNED_CALLS.contains(&fn_name.to_string().as_str()) {
        quote! {
            pub async fn #fn_name(&self, #(#param_idents: crate::interfaces::#network::api::#pallet::calls::#param_types,)* signer: impl subxt::tx::signer::Signer<subxt::PolkadotConfig>) -> Result<subxt::PolkadotConfig::Hash, , crate::error::CallError> {
                let call = crate::interfaces::#network::api::tx().#pallet().#fn_name(#(#param_idents),*);
                (self.client.tx().sign_and_submit_default(&call, signer).await?)
            }

            pub async fn #wait_fn_name(&self, #(#param_idents: crate::interfaces::#network::api::#pallet::calls::#param_types,)* signer: impl subxt::tx::signer::Signer<subxt::PolkadotConfig>) -> Result<subxt::PolkadotConfig::Hash, crate::error::CallError> {
                let call = crate::interfaces::#network::api::tx().#pallet().#fn_name(#(#param_idents),*);

                let stream = self.client.tx().sign_and_submit_then_watch_default(&call, signer).await?;
        ...
        quote! {
            pub async fn #fn_name(&self, #(#param_idents: crate::interfaces::#network::api::#pallet::calls::#param_types,)*) -> Result<subxt::PolkadotConfig::Hash, subxt::error::Error> {
                let call = crate::interfaces::#network::api::tx().#pallet().#fn_name(#(#param_idents),*);
                self.client.tx().create_unsigned(&call)?.submit()
            }

            pub async fn #wait_fn_name(&self, #(#param_idents: crate::interfaces::#network::api::#pallet::calls::#param_types,)*) -> Result<subxt::PolkadotConfig::Hash, subxt::error::Error> {
                let call = crate::interfaces::#network::api::tx().#pallet().#fn_name(#(#param_idents),*);
