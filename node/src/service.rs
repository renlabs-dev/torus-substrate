...
use torus_runtime::{apis::RuntimeApi, interface::OpaqueBlock as Block};
...
pub fn new_partial(config: &Configuration, consensus: Consensus) -> Result<Service, ServiceError> {
...
fn aura_data_provider(
    ...
    Box<
        dyn std::future::Future<
                Output = Result<
                    (
                        sc_consensus_aura::InherentDataProvider,
...
fn build_aura_grandpa_import_queue(
    ...
    let import_queue = sc_consensus_aura::import_queue::<AuthorityPair, _, _, _, _, _>(
        sc_consensus_aura::ImportQueueParams {
            ...
            client,
            create_inherent_data_providers: aura_data_provider(slot_duration),
